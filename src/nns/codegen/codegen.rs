...
fn gen_cuda(module: &MLIRModule, opts: &CodegenOptions) -> NsResult<String> {
    ...
    if opts.enable_fp16 {
        // fp16: emit half-precision kernels with __half / half2 vectorization.
        // Defines NS_DTYPE as __half and NS_FP16 for conditional compilation.
        os.push_str("// fp16 enabled: half-precision kernels\n");
        os.push_str(cuda_backend::CUDA_FP16_HEADER);
    }
...
fn emit_train_core_cuda(
    ...
    let mut os = String::new();
    if enable_fp16 {
        os.push_str("// fp16: ns_train_core using __half / half2 (NS_FP16)\n");
    }
