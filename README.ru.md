...
Скомпилированные программы — обычные нативные бинарники. В тот же toolchain встроен
**NeuralScript** (`.ns`) — типизированный AOT tensor language для CPU/CUDA codegen.

## Философия

Glyphc объединяет явное системное программирование и типизированные ML-графы в
одном прозрачном toolchain: от исходника к нативному артефакту без обязательного
Python- или GC-слоя. Полный manifesto доступен в документе
...
- Flow-sensitive статический анализ: обращение к `List`/`Map` после `.free()`
  (и повторный `.free()`) отклоняется на этапе компиляции, пока переменная не
  переприсвоена
...
- Конкурентность (M:N пул воркеров): `@fn async`, ленивые хендлы `Async<T>`,
  `spawn`/`await`,
...
Вызов `@fn async` возвращает ленивый хендл `Async<T>` (тело не запущено).
`spawn` ставит задачу в общую очередь пула воркеров (M:N: M задач на N
воркерах, N = число ядер, до 8; переопределяется `GLYPH_WORKERS=K`). `await`
ждёт результат: если хендл ещё не запущен — выполняет синхронно, иначе
ожидает; ждущий поток попутно разбирает очередь (work-sharing), поэтому
вложенные `await` внутри async-функций не дедлокат пул. Повторный `await`
берёт кэш результата:
...
- Конкурентность: без GC — хендлы/каналы не освобождаются (списки — через
  `xs.free()`); пул M:N (число воркеров = ядра, ≤ 8, `GLYPH_WORKERS=K`);
  задачи не вытесняются — долгий корень задачи занимает воркера, вложенные
  `await` переключают воркера на очередь; руки `select` — только `recv()`/`await`,
  ожидание — пулинг с шагом ~1 мс; async-generic функций нет;
  `send(&ref)` запрещён (адрес стека)
