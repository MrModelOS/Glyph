...
impl CCodegen {
    ...
    fn compile_inner(
        ...
        writeln!(self.output, "// Generated by glyphc v1.3.0").unwrap();
    ...
    fn emit_expression(&mut self, expr: &Expr) -> Result<(), CodegenError> {
        match expr {
            ...
            Expr::MethodCall {
                ...
                match method.as_str() {
                    ...
                    "free" => {
                        ...
                        if is_map {
                            write!(self.output, "glyph_map_free(&(").unwrap();
                        } else {
                            write!(self.output, "glyph_list_free(&(").unwrap();
                        }
                        self.emit_expression(object)?;
            ...
            Expr::IndexAccess { object, index, .. } => {
                ...
                match self.resolved_map_value(object) {
                    Some(val) => {
            ...
            Expr::MapLiteral(pairs, _) => {
                ...
                for (k, v) in pairs {
                    ...
                    if let Expr::StringLiteral(s, _) = k {
                        ...
                        write!(self.output, "glyph_map_put(&_m, (const char*)(").unwrap();
                        self.emit_expression(k)?;
                        write!(self.output, "), &_mv); }}").unwrap();
                    }
                }
                write!(self.output, " _m; }})").unwrap();
            }
...
Stmt::Assignment { target, value, .. } => {
    ...
    fn emit_builtin_functions(&mut self) {
        ...
        writeln!(self.output, "        e->value = malloc((size_t)m->elem_size);").unwrap();
        writeln!(self.output, "        e->next = m->buckets[i];").unwrap();
        writeln!(self.output, "        m->buckets[i] = e; m->size++;").unwrap();
        writeln!(self.output, "    }}").unwrap();
        ...
        writeln!(self.output, "    for (int64_t i = 0; i < m->bucket_count; i++) {{").unwrap();
        writeln!(self.output, "        GlyphMapEntry* e = m->buckets[i];").unwrap();
        writeln!(self.output, "        while (e) {{ GlyphMapEntry* n = e->next; free(e->key); free(e->value); free(e); e = n; }}").unwrap();
        writeln!(self.output, "    }}").unwrap();
        writeln!(self.output, "    free(m->buckets);").unwrap();
