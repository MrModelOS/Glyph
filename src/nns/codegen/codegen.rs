...
impl CodeGenerator {
    ...
    pub fn generate(&self, module: &MLIRModule, opts: &CodegenOptions) -> NsResult<String> {
        match opts.backend {
            ...
            TargetBackend::Rocm => self.gen_rocm(module, opts),
            TargetBackend::Metal => self.gen_metal(module, opts),
        }
    }
    ...
    /// ROCm backend stub — currently delegates to CUDA.
    /// TODO: replace with HIP/rocBLAS codegen (hip_runtime.h, half-precision via hip_fp16).
    pub fn gen_rocm(&self, module: &MLIRModule, opts: &CodegenOptions) -> NsResult<String> {
        // TODO(ROCm): emit HIP kernel launches and rocBLAS/hipblas handles instead of CUDA.
        gen_cuda(module, opts)
    }

    /// Metal backend stub — currently delegates to CUDA.
    /// TODO: replace with Metal Shading Language + MPS codegen.
    pub fn gen_metal(&self, module: &MLIRModule, opts: &CodegenOptions) -> NsResult<String> {
        // TODO(Metal): emit MSL kernels and Metal/MPS dispatch instead of CUDA.
        gen_cuda(module, opts)
    }
...
// ---------------------------------------------------------------------------
// Free helper stubs for alternative backends (explicit placeholders).
// ---------------------------------------------------------------------------

/// ROCm/HIP backend stub — delegates to CUDA codegen.
/// TODO: emit HIP (hip_runtime.h / rocBLAS) kernels instead of CUDA.
pub fn gen_rocm(module: &MLIRModule, opts: &CodegenOptions) -> NsResult<String> {
    // TODO(ROCm): replace cuda_runtime.h / cuda kernels with HIP equivalents.
    gen_cuda(module, opts)
}

/// Metal backend stub — delegates to CUDA codegen.
/// TODO: emit Metal Shading Language / MPS dispatch instead of CUDA.
pub fn gen_metal(module: &MLIRModule, opts: &CodegenOptions) -> NsResult<String> {
    // TODO(Metal): replace CUDA codegen with MSL/MPS.
    gen_cuda(module, opts)
}
...
fn gen_cuda(module: &MLIRModule, opts: &CodegenOptions) -> NsResult<String> {
    ...
    // TODO(fp16): when opts.enable_fp16 is true, emit half-precision kernels (__half, __half2)
    // and adjust NS_LAUNCH_BLOCKS grid sizing for fp16 vectorization. Currently fp16 only
    // gates this header/comment and is otherwise a no-op placeholder.
    if opts.enable_fp16 {
        os.push_str("// fp16 enabled: half-precision kernels (placeholder)\n");
        os.push_str("#include <cuda_fp16.h>\n");
    }
