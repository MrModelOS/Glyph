...
Статус: v1.2.0. Документ описывает реально работающий синтаксис
...
| `List<T>` | `GlyphList` (fat-struct `{data, len, elem_size}`) | литерал `[..]`, диапазон `..`, `.len()`, `.append()`, `for x in xs`, `++` |
...
Ограничения v1.2: обобщаются только функции (структуры/enum/impl —
...
(иначе — ошибка `Cannot infer type arguments`); `Map` объявлен, но не имеет
рантайм-операций (итерация по `Map` отвергается на этапе проверки типов);
тип `[T; N]` описан, но литерал всегда порождает `List<T>`, поэтому массивы
в v1.2 неконструируемы.
...
Списки — fat-struct `{data, len}` поверх heap-буфера: литералы и диапазоны
выделяют память, `append` делает `realloc`, индексация `arr[i]` возвращает
элемент `T`. Передача списка в функцию копирует структуру (буфер общий;
`append` внутри функции не виден снаружи). Тип элемента литерала выводится
по первому элементу. `let` без аннотации тоже выводит тип (`let xs = [1, 2]`
даёт `List<Int64>`).
...
остальные — громкая ошибка кодгена. `xs.free()` освобождает буфер
(`len` становится `0`). Итерация по `Map` и рантайм `Map` — вне версии
(тайпчекер принимает `for k in map`, кодген пока отклоняет).
