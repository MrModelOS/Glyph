...
- `List<T>`: передача в функцию копирует fat-struct (буфер общий); `==` для списков
  и структур не поддерживается (только скаляры/указатели); `Map` не итерируется
- Конкурентность: без GC — хендлы/каналы/списки не освобождаются; нет `select`,
  таймаутов и async-generic функций; `send(&ref)` запрещён (адрес стека)
- Ошибки компилятора — без спанов (только текст)
