...
Ограничения: рантайм без GC (хендлы, каналы и ячейки payload не
освобождаются); нет `select`/таймаутов; async-generic функции запрещены;
передача `&ref` между потоками — на свой риск (адрес стека).
